...
function getOffset(offset: FloatingOffset, position: PositionHorizontal | PositionVertical): any {
  let offsetValue: any = offset;

  if (typeof offset === "number")
    offsetValue = offset;
  else if (offset.hasOwnProperty("horizontal") && (position === "left" || position === "right"))
    // @ts-ignore
    offsetValue = offset.horizontal;
  else if (offset.hasOwnProperty("vertical") && (position === "top" || position === "bottom"))
    // @ts-ignore
  ...
  if (typeof offsetValue === "number")
    return `${offsetValue}px`;
  else
    return offsetValue;
}

export function useFloating(props: MakeResponsive<UseFloatingProps>): UseFloatingOutput {
  const {
  ...
  if (positionHorizontal === "center")
    transformFunction += "translateX(-50%) ";
  if (positionVertical === "center")
    transformFunction += "translateY(-50%)";

  const style: CSSProperties = {
    top: positionVertical === "top" || positionVertical === "center" ? offsetTop : undefined,
    right: positionHorizontal === "right" ? offsetRight : undefined,
    bottom: positionVertical === "bottom" ? offsetBottom : undefined,
    left: positionHorizontal === "left" || positionHorizontal === "center" ? offsetLeft : undefined,
